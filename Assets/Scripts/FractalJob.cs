...
using UnityEngine;
using Random = UnityEngine.Random;  // для избежания конфликтов
...
public class FractalJob : MonoBehaviour {
	...
	struct UpdateFractalLevelJob : IJobFor {
		...
		public void Execute (int i) {
			...
			// определение верхней оси детали в мировом пространстве
			float3 upAxis = mul(mul(parent.worldRotation, part.rotation), up());
			// ось провисания
			float3 sagAxis = cross(up(), upAxis);  // векторное произведение
			

			float sagMagnitude = length(sagAxis); // длина вектора
			...
			// мой наклон по оси z
			//baseRotation = mul(baseRotation, quaternion.Euler(0f, 0f, 76f));
	...
	FractalPart CreatePart (int childIndex) => new FractalPart {
		maxSagAngle = radians(Random.Range(spinSpeedA, spinSpeedB)),
